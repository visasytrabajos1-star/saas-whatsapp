            ...
            <header className="mb-10 flex justify-between items-end">
                ...
                <div className="flex gap-3">
                    <div className="relative">
                        ...
                        <input type="text" placeholder="Buscar usuarios..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="bg-slate-950 border border-slate-800 rounded-xl py-2 pl-10 pr-4 text-sm focus:ring-2 focus:ring-blue-500 outline-none w-64" />
                    </div>
                </div>
            </header>

            <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-10">
                <MetricCard title="Usuarios Totales" value={stats.total_users} icon={<Users className="text-blue-400" />} trend="+12%" isUp={true} />
                <MetricCard title="Suscripciones Activas" value={stats.active_subscriptions} icon={<TrendingUp className="text-emerald-400" />} trend="+5%" isUp={true} />
                <MetricCard title="MRR (Ingresos/mes)" value={`$${stats.total_revenue.toLocaleString()}`} icon={<DollarSign className="text-yellow-400" />} trend="+18%" isUp={true} />
                <MetricCard title="Mensajes Globales" value={stats.total_messages.toLocaleString()} icon={<Activity className="text-purple-400" />} trend="-2%" isUp={false} />
            </div>

            <section className="bg-slate-950 border border-slate-800 rounded-3xl overflow-hidden shadow-2xl">
                <div className="p-6 border-b border-slate-800 flex justify-between items-center bg-slate-950/50">
                    <h3 className="font-bold flex items-center gap-2"><Users size={18} className="text-slate-500" /> Directorio de Clientes</h3>
                </div>
                                ...
                                <tr key={user.id} className="hover:bg-slate-900/40 transition-colors group">
                                    <td className="px-6 py-4">
                                        <div className="flex items-center gap-3">
                                            <div className="w-8 h-8 rounded-lg bg-blue-600/10 flex items-center justify-center text-blue-500 font-bold text-xs border border-blue-500/20">{user.email?.[0].toUpperCase()}</div>
                                            <div>
                                    ...
                                    <td className="px-6 py-4">
                                        <span className={`text-[10px] px-2 py-0.5 rounded-full font-bold uppercase tracking-wider ${user.plans?.name === 'Pro' ? 'bg-yellow-500/10 text-yellow-500' : user.plans?.name === 'Enterprise' ? 'bg-purple-500/10 text-purple-500' : 'bg-slate-800 text-slate-400'}`}>
                                    ...
                                    <td className="px-6 py-4"><p className="text-xs font-mono text-slate-300">${user.plans?.price_monthly || 0}/mes</p></td>
